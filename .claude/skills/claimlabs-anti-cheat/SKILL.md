...
| Modified browser code changing displayed score | Server doesn't trust displayed score directly — `resolve_run` re-checks bounds | Server checks *bounds*, not the actual gameplay — a modified client that reports a plausible-but-fake score within `max_plausible_score` would not be caught |
| Impossible score for the time played | `resolve_run` checks `p_duration` against `min_run_ms` | Only a floor on duration, and a ceiling on score — no correlation check between the *rate* of scoring and what's physically possible given `FOODS` point values and spawn rate |
