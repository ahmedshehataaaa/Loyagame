...
if (isGit) {
  ...
  if (diff) notes.push(`Uncommitted changes present — confirm this diff matches the requested scope:\n${diff}`);
} else {
  notes.push('No git repo yet — "git diff matches requested scope" cannot be mechanically checked. Compare changed files by hand against what was asked.');
}
...
if (fs.existsSync('docs/security/reward-wheel-compliance.md')) {
  ...
  const touchedRewardCode = diff && /(engine\/config\.js|supabase\/schema\.sql|resolve_run)/i.test(diff);
  if (touchedRewardCode) {
    notes.push('Reward/wheel-related files changed — confirm the security-adversary review ran (see REVIEW.md) before calling this done.');
  }
}
