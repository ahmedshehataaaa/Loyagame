...
process.stdin.on('end', () => {
  ...
  const rules = [
    ...
    [
      /\b(cat|type|Get-Content)\s+.*\.env\b/i,
      'reading a .env file — check this is actually necessary before printing secrets',
    ],
    [
      /(SUPABASE_SERVICE_KEY|ADMIN_KEY|FOODICS_WEBHOOK_SECRET)\s*=\s*['"]/i,
      'a secret value looks like it is being hardcoded/echoed inline, not read from process.env',
    ],
    ...
    [
      /vercel\s+link\b/i,
      'changing the Vercel project link — see ADR 0002, verify the target project by hand',
    ],
    ...
    [
      /\brm\b.*(\.test\.|\.spec\.|[\\/]tests?[\\/])/i,
      'deleting a test file — if this is to make a build pass rather than a genuine cleanup, stop and fix the underlying issue instead',
    ],
  ];
