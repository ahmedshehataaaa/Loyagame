...
process.stdin.on('end', () => {
  ...
  if (ext === '.json') {
    try {
      ...
      blockers.push(`invalid JSON: ${e.message}`);
    }
  }
  ...
  const secretPatterns = [
    [
      /SUPABASE_SERVICE_KEY\s*[:=]\s*['"][A-Za-z0-9._-]{10,}/,
      'hardcoded SUPABASE_SERVICE_KEY (should be process.env only)',
    ],
  ...
  if (
    /\bffn_dev\b|localStorage\.\w+\s*=\s*['"]1['"].*dev/i.test(content) &&
    /engine\//.test(filePath)
  ) {
    warnings.push(
      'looks like a client-side dev/cheat flag pattern (see fastfood-ninja lineage precedent) — confirm this is not shipping to production',
    );
  }
  ...
  if (base === 'vercel.json' || base === 'manifest.webmanifest') {
    try {
      ...
      blockers.push(`${base} is not valid JSON: ${e.message}`);
    }
  }
      ...
      for (const script of ['lint', 'typecheck', 'format:check']) {
        if (pkg.scripts?.[script]) {
          try {
            ...
            warnings.push(`npm run ${script} failed:\n${e.stdout?.toString().trim() || e.message}`);
          }
        }
      }
    } catch {
      /* malformed package.json will already be caught above if it's the edited file */
    }
  }

  if (blockers.length) {
    process.stderr.write(
      `BLOCKED by post-edit-check on ${filePath}:\n- ${blockers.join('\n- ')}\n`,
    );
    process.exit(2);
  }
  if (warnings.length) {
