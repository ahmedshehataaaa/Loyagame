...
function fetchStandings() {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      try { resolve(RIVALS.slice()); } catch { reject(new Error('standings_unavailable')); }
    }, 220);
  });
}
...
export function LeaderboardPage(root) {
  ...
  const tick = setInterval(() => { clockEl.textContent = seasonRemaining(); }, 30000);

  root.append(el('div', { class: 'screen bg-burst' },
    topbar('Standings', { back: '/' }),
    el('section', { class: 'card season' },
  ...
  fetchStandings()
    .then((rivals) => {
      ...
      if (profile) rows.push({ id: profile.id, name: profile.name, score: best, you: true, avatar: profile.avatar });
      ...
      listHost.innerHTML = '';
      if (!ranked.length) {
        listHost.append(emptyState('📊', 'No standings yet', 'Play a round to put yourself on the board.'));
        return;
      }
      ...
      listHost.append(
        ...
        el('ol', { class: 'lb-list' }, ...ranked.map((r) => el('li', {
          class: `lb-row${r.you ? ' lb-row--you' : ''}`,
          'aria-current': r.you ? 'true' : null,
        },
          el('span', { class: 'lb-row__rank', text: r.rank <= 3 ? MEDALS[r.rank - 1] : String(r.rank) }),
          r.avatar
            ? el('img', { class: 'lb-row__avatar', src: r.avatar, alt: '', loading: 'lazy' })
            : el('span', { class: 'lb-row__avatar', 'aria-hidden': 'true',
                style: { display: 'grid', placeItems: 'center', background: '#eee', fontSize: '13px' }, text: '🍔' }),
          el('span', { class: 'lb-row__name' },
            el('span', { text: r.name }),
    ...
    .catch(() => {
      listHost.innerHTML = '';
      listHost.append(emptyState('⚠️', 'Standings unavailable', 'Could not load the board. Try again shortly.',
        el('div', { style: { marginTop: '14px', width: '200px' } },
